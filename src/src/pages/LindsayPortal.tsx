...
const PIN = '2017' // Year Lindsay joined Landmark
...
export default function LindsayPortal() {
  ...
  const [intakeForm, setIntakeForm] = useState<IntakeForm>({
    aiToday: '',
    topChallenge: 'Building comprehensive financial plans efficiently — especially complex planning situations involving divorce, estate, or tax strategy',
    clientType: ['HNW Individuals & Families', 'Institutions', 'Nonprofits & Foundations'],
    sectors: ['Financial Services', 'Healthcare', 'Technology / AI'],
    horizon: '10–20 years',
    notes: 'VP of Advisory Services at Landmark — my work is the full financial planning relationship: income analysis, retirement projections, estate planning, tax efficiency, investment committee participation. I carry the CDFA credential, so I handle a number of divorce financial planning cases — these are often the most complex and emotionally high-stakes engagements. I came through Ernst & Young (audit + tax) and TPG Capital (private equity fund accounting) before joining Landmark in 2017. I think in accounting terms — I care deeply about what the numbers actually say, not just what the summary shows. Fee-only fiduciary. Tax efficiency is a core planning lens for every client.'
  })
        ...
        <div style={{ width: '100%', maxWidth: '380px', textAlign: 'center' }}>
          ...
          <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '20px', padding: '44px 40px 40px' }}>
            ...
            <div style={{ fontSize: '14px', color: GRAY, marginBottom: '36px', lineHeight: '1.5' }}>
              ...
              <span style={{ color: '#444', fontSize: '12px' }}>VP of Advisory Services · Getzville, NY</span>
            </div>
    ...
    return (
      <div style={{
        ...
        <nav style={{
          ...
          background: '#0d0d0d',
          display: 'flex',
          paddingBottom: 'env(safe-area-inset-bottom, 0px)' as string,
        } as React.CSSProperties}>
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => setActiveSection(item.id)} style={{
                ...
                padding: '10px 2px 10px', background: isActive ? `${GOLD}12` : 'none',
                border: 'none', borderTop: `2px solid ${isActive ? GOLD : 'transparent'}`,
  ...
  return (
    <div style={{ maxWidth: '680px' }}>
      ...
      <p style={{ fontSize: '17px', color: LIGHT_GRAY, lineHeight: '1.75', marginBottom: '24px', maxWidth: '540px' }}>
        This portal is built around your practice — comprehensive financial planning, tax-efficient client strategies, and the kind of rigorous analysis that comes from E&Y and TPG Capital before it ever reaches a client. Use it as a working tool.
      </p>

      {/* What we already know — specific to Lindsay */}
      <div style={{ background: `${TEAL}08`, border: `1px solid ${TEAL}20`, borderRadius: '10px', padding: '20px 24px', marginBottom: '36px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {[
            { label: 'Role', value: 'VP of Advisory Services · Landmark Wealth Management · Getzville, NY · with firm since October 2017' },
            { label: 'Credentials', value: 'CPA · CFP® · AIF® · CDFA® · Personal Financial Specialist · MBA, Canisius University' },
            { label: 'Practice model', value: 'Fee-only fiduciary RIA · client service + comprehensive financial plans + investment committee participation' },
            { label: 'Your background', value: 'Ernst & Young (Audit + Tax) → TPG Capital Fort Worth (Private Equity Fund Accounting) → Landmark. Big 4 rigor + institutional PE — you read financials differently than most planners.' },
            { label: 'CDFA specialty', value: 'Certified Divorce Financial Analyst — you handle divorce financial planning engagements that are among the most complex and high-stakes Landmark takes on.' },
            { label: 'Planning lens', value: 'Tax efficiency is core to every plan. You think in accounting terms — what the numbers actually say, not just the summary.' },
            { label: 'Community', value: 'Vice Chair, Board of Directors — Aspire of WNY. Chair of their Investment Committee. Active in FPA, AICPA, and Fi360.' },
            { label: 'Recognition', value: 'AICPA Standing Ovation Award 2019 · Business First 30 Under 30, 2021 · Magna cum laude, Canisius (Accounting + Finance)' },
          ].map(item => (
        ...
        <div style={{ marginTop: '12px', fontSize: '12px', color: '#444' }}>
          Built from your public profile and Landmark's team page. Correct anything that's off in the "About You" section.
        </div>
      </div>
      ...
      <div style={{ background: `${GOLD}08`, border: `1px solid ${GOLD}20`, borderRadius: '10px', padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '200px' }}>
          ...
          <p style={{ fontSize: '14px', color: LIGHT_GRAY, lineHeight: '1.7' }}>
            Try Chat with Kit — ask anything about a client planning scenario, a CDFA case, or a question you'd normally spend 30 minutes researching. That's the fastest way to understand what this does for your day.
          </p>
        </div>
        <button
  ...
  return (
    <div style={{ maxWidth: '680px' }}>
      ...
      <div style={{ background: `${GOLD}08`, border: `1px solid ${GOLD}20`, borderRadius: '8px', padding: '12px 16px', marginBottom: '28px', fontSize: '13px', color: LIGHT_GRAY, lineHeight: '1.6' }}>
        <span style={{ color: GOLD, fontWeight: 700 }}>Try it with a client holding.</span> Pick any company a client has asked about recently and run the brief — see what it surfaces from a fee-only fiduciary lens. That's the fastest way to see whether this saves you prep time.
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '28px' }}>
        {[
          { label: 'Large cap example', value: 'AAPL', sub: 'Apple — commonly held by HNW clients' },
          { label: 'Large cap example', value: 'MSFT', sub: 'Microsoft — AI infrastructure exposure' },
          { label: 'Sector example', value: 'V', sub: 'Visa — financial services, high margins' },
          { label: 'Healthcare example', value: 'JNJ', sub: 'Johnson & Johnson — dividend + defensive' },
          { label: 'Sector example', value: 'BRK-B', sub: 'Berkshire Hathaway — value + diversification proxy' },
        ].map(s => (
          <a
            key={s.value}
      ...
      <div style={{ background: `${TEAL}08`, border: `1px solid ${TEAL}20`, borderRadius: '10px', padding: '20px 24px' }}>
        ...
        <p style={{ fontSize: '13px', color: LIGHT_GRAY, lineHeight: '1.7' }}>
          Every brief includes a section specific to Landmark — how this stock fits your planning practice, what clients with positions are probably asking, and what a CPA-trained fiduciary sees that sell-side coverage misses. It knows your credentials and your firm's strategy.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: '680px' }}>
      ...
      <div style={{ background: `${TEAL}08`, border: `1px solid ${TEAL}20`, borderRadius: '8px', padding: '12px 16px', marginBottom: '28px', fontSize: '13px', color: LIGHT_GRAY, lineHeight: '1.6' }}>
        <span style={{ color: TEAL, fontWeight: 700 }}>The planning leverage section is written for you.</span> How a CPA-trained advisor with a full-service planning practice uses AI to do more — not less — of the work that requires expert judgment. The answer isn't "AI replaces you." It's "you do what only you can do, faster and at greater scale."
      </div>
  ...
  if (saved) {
    return (
      <div style={{ maxWidth: '560px', textAlign: 'center', paddingTop: '40px' }}>
        ...
        <h2 style={{ fontSize: '28px', fontWeight: 900, marginBottom: '12px' }}>Got it, Lindsay.</h2>
        <p style={{ fontSize: '16px', color: LIGHT_GRAY, lineHeight: '1.7', marginBottom: '28px' }}>
          Your context is saved. Kit now knows your planning practice, your credentials, and what matters to you at Landmark. Every conversation starts from there.
        </p>
        <button
          onClick={() => onNavigate('chat')}
          style={{
  ...
  return (
    <div style={{ maxWidth: '600px' }}>
      ...
      <p style={{ fontSize: '15px', color: LIGHT_GRAY, lineHeight: '1.7', marginBottom: '28px' }}>
        We've pre-loaded what we know about you and Landmark from public sources. Review it, correct anything that's off, and add what only you know. The more accurate this is, the more specifically every tool speaks to your planning practice.
      </p>
    ...
    loadPersistedHistory().then(history => {
      if (history.length > 0) {
        ...
        setMessages([{
          role: 'assistant',
          content: intake
            ? "Hey Lindsay — I have your context loaded, including what you just filled in. Ask me anything — a client planning scenario, a CDFA case, tax strategy question, or anything else on your mind."
            : "Hey Lindsay — I'm Kit. Ask me anything — a complex financial planning scenario, a divorce case question, how AI fits into a planning practice, or whatever's in front of you right now."
        }])
      }
    ...
    <div style={{ maxWidth: '680px', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 160px)', minHeight: '500px' }}>
      ...
      <div style={{
        ...
        <textarea
          ...
          onKeyDown={handleKey}
          placeholder={messages.filter(m => m.role === 'user').length > 0 ? '' : "Ask about a client scenario, a CDFA case, tax strategy, anything on your mind..."}
          rows={2}
